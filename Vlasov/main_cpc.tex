...
\begin{abstract}
  %what we have done HLE V Solver
  ...
  %No numerical method for general wave. since the method is not general at all. only works for narrow-banded wave.
  %direct use chorus as benchmark. since it is the only successful case, can not even simulate BOT in current framework.
  ...
  % - The code is applied to study the onset of the whistler-mode chorus in the Earth's inner magnetosphere with realistic parameters. It shows the wave growth in both linear and nonlinear stages and accurately gives the nonlinear frequency chirping of the chorus. The code also provides resonant particle phase-space dynamics with high efficiency and resolution. 
  %+the key for inhomogeneous one, it is ...
  %+ at least two benifits
